...
export class ExpensesService {
   ...
   *   GET /expenses?status=payé&month=2026-06
   */
  ...
  /** All expenses submitted by a specific staff member (e.g. teacher reimbursements). */
  getExpensesByStaff(submittedById: string): Observable<Expense[]> {
    return this.getExpenses({ submittedById });
  }

  /** All expenses that are still pending approval / payment. */
  ...
  /**
   * Partial update (PATCH) — only the supplied fields are changed.
  ...
  /**
   * Marks an expense as paid and stamps today as the paid date.
   * Uses PATCH so all other fields remain untouched.
   */
  markAsPaid(
    id: string,
  ...
  // ─── Reporting / aggregation (pure, synchronous) ───────────────────────────

  /**
   * Computes a financial summary from an in-memory list of expenses.
   ...
   * Sorted descending by `totalAmount` for easy charting.
   */
    ...
    return [...map.values()]
      .map((entry) => ({
        ...
        percentage: grandTotal > 0
          ? Math.round((entry.totalAmount / grandTotal) * 100 * 10) / 10
          : 0,
      }))
  ...
  /**
   * Returns a live summary observable for all expenses in a given month.
   */
  ...
  /**
   * Returns a live category breakdown observable for a given month.
   */
  ...
  /**
   * Maps an `ExpenseFilters` object to Angular `HttpParams`.
   *
   * Exact-match fields are passed directly as query params.
   * Date range fields use json-server's `_gte` / `_lte` operators.
   */
